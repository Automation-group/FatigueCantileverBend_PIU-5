...
\begin{flushleft}
	...
	$M$ - изгибающий момент в Н$\cdot$м,\newline
	$E$ - модуль Юнга в Па,\newline
	$J$ - момент инерции сечения в кг$\cdot$м$^2$\newline
\end{flushleft}

Поскольку угол $\varphi$ выражен в радианах, то при малых углах прогиба,
менее 0.2 радиан (11.31$\degree$), \mbox{$\tg \, \varphi \approx \varphi$} при
погрешности не более 2\% и поэтому имеем простую
...
\begin{align}
	\label{eq:iint_deflection}
  	\nu &=  \left| \int\limits_{0}^{L} \int\limits_{0}^{L}  \frac{M(z)}{E(z) \cdot J(z)} \,dz \,dz \right|
\end{align}
...
В процессе численного интегрирования определяется значение
максимального механического напряжения $\sigma_{max}$ в рабочей части образце по
формуле \eqref{eq:sigma_max}.
...
координаты $z$, а $J(z)$ зависит от образца с рычагом и
направления приложения силы. То есть возникает четыре варианта
при вычислении величины прогиба $\nu$ в зависимости от формы сечения рычага -
круглой или прямоугольной и направления приложения силы - по оси $x$
или по оси $y$.

\newpage
\subsection{Определение \texorpdfstring{$M(z)$\text{, }$E(z)$\text{ и }$J(z)$}{Lg} для образца и рычага}
...
то точность расчёта упадёт. Если в окне программы поставить галочку на против пункта
$"$Шаг интегрирования$"$ то можно задать шаг интегрирования вручную.

Изначально значение силы $P$ неизвестно а задано только механическое напряжение $\sigma$,
поэтому в программе модуль силы $P$ берётся равным единице. В процессе расчёта
...
\begin{align}
	...
	\nu_\tau &=  \left| \sum_{i=0}^{S_{int}} \sum_{i=0}^{S_{int}} \frac{M(z)}{E(z) \cdot J(z)} \cdot \Delta z \cdot \Delta z \right|
\end{align}
...
Если в окне программы поставить галочку на против пункта $"$Прогиб при силе один Ньютон$"$ то
...
Если в окне программы поставить галочку у пункта $"$Параметры калибровки$"$ то появится
