...
\usepackage{subcaption} % вставка нескольких изображений в один объект
\usepackage{amsmath,amssymb} % математические формулы, математические символы
\usepackage{textcomp, gensymb} % спецсимволы
...
\definecolor{linkcolor}{HTML}{0000FF} % цвет ссылок в HEX(RGB)
\definecolor{urlcolor}{HTML}{0000FF} % цвет гиперссылок в HEX(RGB)
\definecolor{bibicolor}{HTML}{0000FF} % цвет ссылок на источники в HEX(RGB)
\hypersetup{
			pdfstartview=FitH,
			linkcolor=linkcolor,
			urlcolor=urlcolor,
			citecolor=bibicolor,
			colorlinks=true} % настройка цветов для выделения ссылок
